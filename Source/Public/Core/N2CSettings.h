...
/**
 * Clean editor preferences for the Back2Dead Node2Code fork.
 *
 * This class is intentionally small and is the only object registered under
 * Editor Preferences -> Plugins -> Node2Code. The original UN2CSettings class is
 * still kept for legacy NodeToCode translation/LLM internals, but it is no
 * longer registered as the visible editor preferences page.
 */
...
public:
    ...
    /** Automatically format functions changed by N2C import using Blueprint Assist when that plugin exists and is enabled. */
...
UCLASS(Config = NodeToCode, DefaultConfig, meta = (Category = "Plugins", DisplayName = "Node2Code"))
...
public:
    ...
    /** Automatically format functions changed by N2C import using Blueprint Assist when that plugin exists and is enabled. */
    UPROPERTY(Config, EditAnywhere, BlueprintReadOnly, Category = "Node to Code | Blueprint Assistant Extension",
        meta = (DisplayName = "Auto Format Imported Functions",
                ToolTip = "If enabled, Node to Code checks whether Blueprint Assist exists and is enabled after import, then formats every function graph changed by the patch."))
    bool bAutoFormatImportedFunctionsWithBlueprintAssist = false;
