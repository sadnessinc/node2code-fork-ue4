...
TSharedPtr<FJsonObject> FN2CSerializer::PinToJsonObject(const FN2CPinDefinition& Pin)
{
    ...
    // Always write type, including Exec.
    // Back2Dead fork note: ParsePinFromJson expects this field, so omitting it for Exec pins
    // breaks round-trip JSON readers and patch tooling.
    JsonObject->SetStringField(TEXT("type"), 
