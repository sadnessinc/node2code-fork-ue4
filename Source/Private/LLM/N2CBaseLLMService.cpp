    ...
    const FOnLLMResponseReceived& OnComplete)
{
    ...
    // ============================================================
    // RAW EXPORT WITHOUT LLM
    // ============================================================

    const FString SaveDir =
        FPaths::ProjectSavedDir() / TEXT("NodeToCode/RawExports");

    IFileManager::Get().MakeDirectory(*SaveDir, true);

    const FString Timestamp =
        FString::Printf(TEXT("%lld"), FDateTime::Now().GetTicks());

    const FString RawPayloadPath =
        SaveDir / FString::Printf(TEXT("N2C_RawPayload_%s.json"), *Timestamp);

    const bool bSavedRawPayload = FFileHelper::SaveStringToFile(
        JsonPayload,
        *RawPayloadPath,
    ...
    {
        FN2CLogger::Get().Log(
            FString::Printf(TEXT("Raw NodeToCode JSON saved: %s"), *RawPayloadPath),
    ...
    else
    {
        FN2CLogger::Get().LogError(
            FString::Printf(TEXT("Failed to save raw NodeToCode JSON: %s"), *RawPayloadPath),
            TEXT("BaseLLMService")
        );
    }

    // Если нужен ещё и полный formatted request, раскомментируй этот блок.
    /*
    const FString FormattedPayload = FormatRequestPayload(JsonPayload, SystemMessage);

    const FString FormattedPayloadPath =
        SaveDir / FString::Printf(TEXT("N2C_FormattedLLMRequest_%s.json"), *Timestamp);

    FFileHelper::SaveStringToFile(
        FormattedPayload,
        *FormattedPayloadPath,
        FFileHelper::EEncodingOptions::ForceUTF8
    );
    */

    // ВАЖНО:
    // Останавливаемся здесь, чтобы плагин НЕ отправлял запрос в Anthropic/OpenAI/LM Studio.
    // Тебе нужен только JSON, он уже сохранён.
    const bool bExecuted = OnComplete.ExecuteIfBound(
        FString::Printf(
            TEXT("{\"raw_export_saved\":true,\"path\":\"%s\"}"),
            *RawPayloadPath.Replace(TEXT("\\"), TEXT("\\\\"))
        )
    );
    ...
    // ============================================================
    // ORIGINAL LLM REQUEST CODE DISABLED BY RETURN ABOVE
    // ============================================================
